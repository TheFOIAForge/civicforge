...
export default function SettingsPage() {
  ...
  useEffect(() => {
    const stored = localStorage.getItem("checkmyrep_api_key");
    if (stored) {
      setSavedKey(stored);
      setApiKey(stored);
    }
    // Load email config
    const emailConfig = localStorage.getItem("checkmyrep_email_config");
    if (emailConfig) {
  ...
  function handleSave() {
    ...
    localStorage.setItem("checkmyrep_api_key", trimmed);
    setSavedKey(trimmed);
    setTestResult(null);
  }

  function handleRemove() {
    localStorage.removeItem("checkmyrep_api_key");
    setApiKey("");
    setSavedKey("");
    setTestResult(null);
  }
  ...
  function handleClearContacts() {
    localStorage.removeItem("checkmyrep_contacts");
  }

  function handleClearCampaigns() {
    localStorage.removeItem("checkmyrep_campaigns");
  }
        ...
        <div className="flex flex-wrap gap-3">
          <button
            onClick={() => {
              ...
              localStorage.setItem("checkmyrep_email_config", JSON.stringify(config));
              setEmailSaved(true);
            }}
          ...
          {emailSaved && (
            <button
              onClick={() => {
                localStorage.removeItem("checkmyrep_email_config");
                setEmailApiKey("");
                setSenderEmail("");
                setEmailSaved(false);
              }}
