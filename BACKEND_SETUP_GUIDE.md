# CivicForge Backend Setup Guide

Step-by-step beginner instructions for getting all API keys, setting up the server, and wiring real data into CivicForge.
...
Right now CivicForge runs entirely in the browser with mock data. To get real data, we need:
...
3. Enter these details:
   - **Project name**: `civicforge` (or whatever you want)
...
- **Name**: Your name
- **Email**: Your email
- **Organization**: `CivicForge` (or personal)
...
In your terminal:
```bash
cd /Users/macbook/Desktop/civicforge
```
...
```bash
mkdir -p /Users/macbook/Desktop/civicforge/src/app/api
```
...
git init
git add -A
git commit -m "Initial CivicForge build"
```

Then create a repo on GitHub:
1. Go to https://github.com/new
2. Name it `civicforge`
...
```bash
git remote add origin https://github.com/YOUR_USERNAME/civicforge.git
git branch -M main
...
3. Find and select your `civicforge` repository
...
Vercel will build and deploy your app. You'll get a URL like `civicforge.vercel.app`.
...
To make CivicForge available at `civic.thefoiaforge.org`:
...
Once DNS propagates, visit `https://civic.thefoiaforge.org` — you should see CivicForge.
